...
header_txt = ["AU", "TI", "PY", "SO", "DI", "AB", "DE"]

columns_to_replace_txt = [
    ("AU", 0),
    ("TI", 1),
    ("PY", 2),
    ("SO", 3),
...
def merge_and_process_files_in_csv(scopus_path, wos_path, output_path):
    scopus_df = pd.read_csv(scopus_path, on_bad_lines="skip", dtype=str)
    wos_df = pd.read_csv(wos_path, on_bad_lines="skip", dtype=str)

    df = pd.concat([scopus_df, wos_df], ignore_index=True)
...
def merge_and_process_files_in_txt(scopus_path, wos_path, output_path):
    scopus_df = pd.read_csv(scopus_path, sep="\t", on_bad_lines="skip", dtype=str)
    wos_df = pd.read_csv(wos_path, sep="\t", on_bad_lines="skip", dtype=str)
    ...
    for column in df.columns:
        if df[column].dtype == "object":
            ...
            if column == "PT":
                df.loc[:, column] = df[column].apply(lambda x: x.leng)
