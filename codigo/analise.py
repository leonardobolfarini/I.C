...
def process_scopus_data(df, scopus_path, scopus_path_txt):
    data = []
    with open(scopus_path) as csv_scopus_file:
        for i, row in enumerate(csv.reader(csv_scopus_file, delimiter=',')):
            if i > 0:
                data.append(row)
    
    with open(scopus_path_txt, 'w', newline='') as scopus_path_txt:
        csv_writer = csv.writer(scopus_path_txt, delimiter='\t')
...
def process_files():
    ...
    scopus_df = pd.read_csv(scopus_path_csv, sep=',', on_bad_lines='skip')
    scopus_df = process_scopus_data(scopus_df, scopus_path_csv, scopus_path_txt)
    ...
    wos_df = pd.read_csv(wos_path_txt, sep='\t', on_bad_lines='skip')
    ...
    return send_file(output_csv_path, as_attachment=True, download_name='all_in_one.csv')
        # send_file(output_txt_path, as_attachment=True, download_name='all_in_one.txt')
'''
